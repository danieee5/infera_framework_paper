...
Runner principal del Protocolo C (INFERA).
...
Requisitos de medicion (reusa EXP1):
...
Uso tipico:
  python infera_session_runner.py \
      ...
      --out results/run_FP16_naive_rep1.jsonl
"""
...
def run_session(args):
    ...
    # --- Warmup: descartar N requests antes de medir (protocolo EXP1) ---
...
def main():
    ap = argparse.ArgumentParser(description="INFERA Protocolo C — runner de sesion incremental")
    ...
    ap.add_argument("--tasks", default="session_tasks.json")
    ap.add_argument("--out", required=True)
    ap.add_argument("--compaction-threshold", type=int, default=4000,
                    help="tokens de contexto acumulado que disparan compactacion (brazo compaction)")
    ap.add_argument("--device-index", type=int, default=0)
    ap.add_argument("--warmup", type=int, default=5)
    args = ap.parse_args()

    print(f"=== INFERA Protocolo C | {args.quant} | {args.arm} | rep {args.rep} ===")
    run_session(args)
